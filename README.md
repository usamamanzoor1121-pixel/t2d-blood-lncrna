...
Or via Nextflow (**note:** written and included for reproducibility/orchestration
clarity, but not executed end-to-end in this session — this sandboxed WSL
environment's Java 11 is below Nextflow's minimum requirement (17+), and installing
a newer JDK stalled in this particular session. To actually run it: install a JDK
≥17, then `curl -s https://get.nextflow.io | bash && ./nextflow run main.nf`. The
five scripts it orchestrates are the same ones already validated by direct
execution above — this is a packaging/reproducibility layer, not unverified logic):
